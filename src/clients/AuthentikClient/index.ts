...
import {
  ...
  AuthentikServerVersion,
  CreateTeamRequest,
  CreateUserRequest,
  GetGroupInfoRequestOptions,
  GetGroupInfoResponse as GetGroupInfoResponse,
  GetTeamsListOptions as GetGroupsListOptions,
  GetTeamsListResponse as GetGroupsListResponse,
...
export class AuthentikClient {
  ...
  constructor() {
    if (!this.AxiosBaseConfig.baseURL) throw new Error('Authentik Backend URL is Invalid!')

    if (!this.AUTHENTIK_TOKEN) throw new Error('Authentik Token is Invalid!')
  }
  ...
  public static validateAuthentikConnection = async () => {
    ...
    try {
      ...
      if (
        !version.isAtLeast(AuthentikClient.MIN_SERVER_VERSION) ||
        !version.isAtMost(AuthentikClient.MAX_SERVER_VERSION)
      )
  ...
  public validateServiceExistance = async () => {
    ...
    for (const rootTeamName in ENABLED_SERVICE_TEAMS) {
      ...
      try {
        ...
        if (
          e instanceof AuthentikClientError &&
          e.code == AuthentikClientErrorType.GROUP_DUPLICATE_EXISTS
        ) {
          try {
  ...
  public getUserList = async (options: GetUserListOptions): Promise<GetUserListResponse> => {
    ...
    if (options?.page) RequestConfig.params.page = options.page

    if (options?.search) RequestConfig.params.search = options.search
  ...
  public getRootTeamsForUsername = async (
    username: string
    ...
    try {
      ...
      for (const group of res.data.results) {
        ...
        if (!group.attributes?.peoplePortalCreation) continue
  ...
  public getGroupsList = async (options: GetGroupsListOptions): Promise<GetGroupsListResponse> => {
    ...
    try {
      while (collectedTeams.length < limit && hasMorePages) {
        ...
        if (options.includeUsers) RequestConfig.params.include_users = options.includeUsers
        ...
        for (let i = currentCursor.authentikIndex; i < results.length; i++) {
          ...
          const parentPk =
            entry.parent ??
            entry.parents?.[0] ??
            null /* 01-19-2026 (@atheesh): Filtering Patches to support Authentik v2025.12+ */
          const isMatch =
            entry.attributes.peoplePortalCreation && (options.subgroupsOnly ? parentPk : !parentPk)
    ...
    try {
      ...
      return {
        ...
        parentPk:
          res.data.parent ??
          res.data.parents?.[0] ??
          null /* People Portal Legacy Single Parent Patch */,
        parentInfo: res.data.parents_obj
          ? res.data.parents_obj[0]
          : null /* Authentik 2025.12+ Only */,
        attributes: res.data.attributes,
      ...
      if (
        ...
        e.response?.headers['x-powered-by'] === 'authentik'
      )
        throw new AuthentikClientError(AuthentikClientErrorType.GROUP_NOT_FOUND)
   ...
   */
  private updateGroupAttributes = async (
    teamId: string,
    updatePayload: Partial<TeamAttributeDefinition>
  ): Promise<boolean> => {
    const groupInfo = await this.getGroupInfo(teamId)
    const RequestConfig: any = {
      ...this.AxiosBaseConfig,
      method: 'patch',
      url: `/api/v3/core/groups/${teamId}/`,
   ...
   */
  public updateRootTeamSettings = async (
    teamId: string,
    teamSettings: { [key: string]: EnabledRootSettings }
  ): Promise<boolean> => {
    return await this.updateGroupAttributes(teamId, { rootTeamSettings: teamSettings })
  }
   ...
   * @returns True if update was successful
   */
  public updateBindlePermissions = async (
    teamId: string,
    bindlePermissions: { [key: string]: EnabledBindlePermissions }
  ): Promise<boolean> => {
    return await this.updateGroupAttributes(teamId, { bindlePermissions })
  }
  ...
  public flagGroupForDeletion = async (teamId: string): Promise<boolean> => {
    return await this.updateGroupAttributes(teamId, { flaggedForDeletion: true })
  }
   ...
   */
  public updateGroupInformation = async (
    teamId: string,
  ...
  public updateUserAttributes = async (
    userId: number,
    updatePayload: Partial<UserAttributeDefinition>
  ): Promise<boolean> => {
  ...
  public createNewUser = async (request: CreateUserRequest): Promise<boolean> => {
    ...
    const username = request.email.replace('@terpmail.umd.edu', '')
    const RequestConfigAddUser: any = {
      ...this.AxiosBaseConfig,
      method: 'post',
      url: '/api/v3/core/users/',
      data: {
    ...
    try {
      ...
      /* Set User Password */
      const RequestConfigSetPassword: any = {
        ...this.AxiosBaseConfig,
        method: 'post',
  ...
  public createNewTeam = async (request: CreateTeamRequest): Promise<GetGroupInfoResponse> => {
    ...
    if (
      ...
      (Object.values(ServiceSeasonType) as string[]).includes(request.attributes.seasonType)
    ) {
    ...
    const teamName = request.serviceTeamName
      ? request.serviceTeamName
      : sanitizeGroupName(
          `${attr.friendlyName.replaceAll(' ', '')}${attr.seasonType}${attr.seasonYear}`
        )
    ...
    try {
      ...
      if (!(e instanceof AuthentikClientError)) throw e
    }

    if (teamExists) throw new AuthentikClientError(AuthentikClientErrorType.GROUP_DUPLICATE_EXISTS)
    ...
    const RequestConfig: any = {
      ...
      data: {
        is_superuser: false,
        name:
          request.parent && !request.serviceTeamName
            ? `${sanitizeGroupName(request.parentName!)}${teamName}`
            : teamName,
