...
import {
  GiteaAPITeamDefinition,
  GiteaAPIUserDefinition,
  GiteaBranchProtection,
  GiteaHookRepositoryTrigger,
  GiteaRepository,
  GiteaRepositoryBrief,
} from './models'
...
export class GiteaClient implements SharedResourceClient {
  ...
  private readonly supportedBindles: BindlePermissionMap = {
    ...
    'repo:brprotect-override': {
      ...
      description:
        'Dangerous: Allows members to bypass all Branch Protections thereby, significantly reducing code quality',
    },
  }

  constructor() {
    if (!this.GiteaBaseConfig.baseURL) throw new Error('Gitea Backend URL is Invalid!')

    if (!this.GITEA_TOKEN) throw new Error('Gitea Token is Invalid!')
  }
  ...
  public async handleOrgBindleSync(
   ...
   * @returns
   */
  private async handleTeamBindleSync(
    ...
    if (team.attributes?.flaggedForDeletion === true) {
      try {
        ...
        const DeleteTeamRequestConfig: any = {
        ...
        if (!(error instanceof GiteaClientResourceNotExists)) {
          ...
          console.error(
            `[GiteaClient] Failed to delete team ${teamSharedResourceId}:`,
            error.message
          )
        }
      }
  ...
  public async handleRepoProvisioning(repoEvent: GiteaHookRepositoryTrigger) {
    ...
    const DeleteCollabRequestConfig: any = {
      ...this.GiteaBaseConfig,
      method: 'delete',
    ...
    const PatchRepoRequestConfig: any = {
      ...this.GiteaBaseConfig,
      method: 'patch',
  ...
  public async handleBranchProtectionSync(
    ...
    if (!repositories) repositories = await this.getOrgRepositories(team.name)
  ...
  private async applyBranchProtections(
    ...
    for (const repositorynt of repositories) {
      ...
      if (repository.empty === true) continue

      const updateRequestConfig: any = {
        ...this.GiteaBaseConfig,
        method: 'patch',
  ...
  private async ensureTeamExists(req: GiteaTeamCreateRequest): Promise<boolean> {
    try {
      ...
      if (
        teamInfo.description !== req.description ||
      ...
      if (!(error instanceof GiteaClientResourceNotExists)) throw error
    }
  ...
  private async ensureOrganizationExists(req: GiteaOrgCreateRequest): Promise<boolean> {
    try {
      ...
      if (
        orgInfo.full_name != req.displayName ||
        orgInfo.website != req.orgWebsite ||
  ...
  private async getTeamInfo(orgName: string, teamName: string): Promise<GiteaAPITeamDefinition> {
    ...
    if (matchedTeams.length < 1) throw new GiteaClientResourceNotExists("Team Doesn't Exist")
