...
export class ATSController extends Controller {
  ...
  async getAllRecruitingTeams() {
    ...
    const recruitingTeams: any[] = await TeamRecruitingStatus.find({ isRecruiting: true })
      .lean()
      .exec()
    ...
    const allConfigs = await SubteamConfig.find({ subteamPk: { $in: allSubteamPks } })
      .lean()
      .exec()
    ...
    for (const team of recruitingTeams) {
      try {
        ...
        if (
          ...
          e.code === AuthentikClientErrorType.GROUP_NOT_FOUND
        ) {
          console.warn(
            `Recruiting Team with PK ${team.teamPk} not found in Authentik. Cleaning up.`
          )
  ...
  async getTeamDetails(@Path() teamId: string) {
    try {
      ...
      if (
        err instanceof AuthentikClientError &&
        err.code === AuthentikClientErrorType.GROUP_NOT_FOUND
      ) {
        this.setStatus(404)
        return {
          error: 'TeamNotFound',
  ...
  ): Promise<ATSApplicationDetails | { error: string; message: string }> {
    try {
  ...
  async updateApplicationStage(
    ...
    try {
      ...
      if (currentStage === stage) {
        return {
          message: 'Stage already set',
          application: { id: application._id, stage: application.stage },
        }
      }
      ...
      const VALID_TRANSITIONS: Record<string, ApplicationStage[]> = {
        ...
        [ApplicationStage.INTERVIEW]: [
          ApplicationStage.POTENTIAL_HIRE,
      ...
      if (!allowed.includes(stage)) {
        ...
        return {
          error: 'BadRequest',
          message: `Invalid transition from ${currentStage} to ${stage}`,
        }
      }
      ...
      if (stage === ApplicationStage.INTERVIEW) {
        ...
        if (
          !interviewGuidelines ||
          interviewGuidelines.length < 50 ||
          interviewGuidelines.length > 500
        ) {
          this.setStatus(400)
          return {
            error: 'BadRequest',
            message: 'Interview Guidelines are required (50-500 characters)',
          }
        }
      }
      ...
      if (stage === ApplicationStage.HIRED) {
        if (!hiredRole || !hiredSubteamPk) {
          ...
          return {
            error: 'BadRequest',
            message: 'Hired Role and Subteam are required for Hired stage',
          }
        }
    ...
    try {
      ...
      const results = await Promise.all(
        applications.map(async (app: any) => {
          ...
          const rolePreferencesWithNames = await Promise.all(
            (app.rolePreferences || []).map(async (pref: any) => {
  ...
  @Security('ats_otp')
  async applyToTeam(@Body() body: ATSTeamApplicationRequest, @Request() request: express.Request) {
    try {
      ...
      if (!userEmail) {
        ...
        return {
          error: 'Unauthorized',
          message: 'User session not found. Please verify your email.',
        }
      }
      ...
      for (const pref of rolePreferences) {
        ...
        if (
          !subteamConfig ||
          !subteamConfig.isRecruiting ||
          !subteamConfig.roles.includes(pref.role)
        ) {
          this.setStatus(400)
          return {
            error: 'InvalidRole',
      ...
      for (const subteamPk of teamStatus.recruitingSubteamPks) {
        ...
        if (config && config.isRecruiting) {
          if (config && config.isRecruiting) {
            for (const pref of rolePreferences) {
              if (pref.subteamPk === subteamPk) {
                // Only check if this preference belongs to this subteam
      ...
      try {
        const isValidSignature = await validateS3FileSignature(profile.resumeUrl, [
          FILE_SIGNATURES.PDF,
        ])
        if (!isValidSignature) {
          // Delete invalid file
      ...
      await Applicant.findByIdAndUpdate(updatedApplicant._id, {
  ...
  async processHiredStageTransition(
    ...
    this.authentikClient
      .getUserInfoFromEmail(applicantEmail)
      .then(async (user: UserInformationBrief) => {
        /* We didn't fail so, Add the Member to the Team & Send Email */
        await this.orgController.addTeamMember(application.hiredSubteamPk!, {
          userPk: +user.pk,
  ...
  async addSubteamToRecruiting(@Path() teamId: string, @Path() subteamId: string) {
    ...
    if (updatedTeam.recruitingSubteamPks.length > 0 && !updatedTeam.isRecruiting) {
      await TeamRecruitingStatus.updateOne({ teamPk: teamId }, { isRecruiting: true })
  ...
  async removeSubteamFromRecruiting(@Path() teamId: string, @Path() subteamId: string) {
    ...
    if (updatedTeam.recruitingSubteamPks.length === 0 && updatedTeam.isRecruiting) {
      await TeamRecruitingStatus.updateOne({ teamPk: teamId }, { isRecruiting: false })
