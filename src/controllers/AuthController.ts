...
export class AuthController extends Controller {
  ...
  async getUserInfo(@Request() req: express.Request): Promise<UserInfoResponse> {
    if (!req.session.accessToken || !req.session.authorizedUser) throw new Error('Unauthorized')

    const userInfo = await OpenIdClient.getUserInfo(
      req.session.accessToken,
      req.session.authorizedUser.sub
    )
  ...
  async handleLogin(
    ...
    if (redirect_uri) {
      ...
      if (state) req.session.tempsession.state = state
    }
    ...
    const authFlowResponse = OpenIdClient.startAuthFlow()
    if (!authFlowResponse) throw new Error('Failed to Compute OIDC Redirect URL')
  ...
  async handleRedirect(@Request() req: express.Request) {
    try {
      ...
      const authorizationStamp = await OpenIdClient.issueAuthorizationStamps(
        new URL(fullURL),
      ...
      if (authorizationStamp.idToken) req.session.idToken = authorizationStamp.idToken
      ...
      if (tempsession?.redirect_uri) {
        ...
        if (redirectUri.origin === currentOrigin && redirectUri.pathname.startsWith('/api/docs')) {
          ...
          if (tempsession.state) hashParams.append('state', tempsession.state)
          ...
          if (tempsession.state) redirectUri.searchParams.append('state', tempsession.state)
        }
  ...
  async otpVerifyRequest(@Body() body: OtpVerifyRequest, @Request() req: express.Request) {
    ...
    if (
      !tempsession.otp ||
      !tempsession.otpEmail ||
      !tempsession.otpExpiry ||
