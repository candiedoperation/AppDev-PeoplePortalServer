...
export async function expressAuthentication(
  ...
  try {
    if (securityName == 'oidc') return await oidcAuthVerify(request, scopes)
    else if (securityName == 'bindles') return await bindlesAuthVerify(request, scopes)
    else if (securityName == 'executive') return await executiveAuthVerify(request, scopes)
    ...
    } else throw new Error('Invalid Security Name!')
  } catch (e) {
...
async function oidcAuthVerify(request: express.Request, scopes?: string[]): Promise<boolean> {
  try {
    ...
    if (!authToken) return Promise.reject(new ResourceAccessError(401, 'No Token Provided'))
    ...
    if (
      ...
      try {
        ...
        if (newTokens.refreshToken) request.session.refreshToken = newTokens.refreshToken
...
export async function executiveAuthVerify(
  ...
  if (scopes && scopes.includes('su:exclusive')) {
    ...
    return Promise.reject(
      new ResourceAccessError(403, 'This action is restricted to Superusers only!')
    )
  }
  ...
  try {
    ...
    const isExecutive = userTeams.teams.some(
      (team) => team.name === 'ExecutiveBoard' && !team.flaggedForDeletion
    )
  ...
  return Promise.reject(
    new ResourceAccessError(
      403,
      'You must be an Executive Board Member or a Superuser to perform this action!'
    )
  )
}
...
async function bindlesAuthVerify(request: express.Request, scopes?: string[]): Promise<boolean> {
  ...
  if (!scopes || scopes.length === 0) {
    return Promise.reject(
      new ResourceAccessError(403, 'Bindle Security Check failed: No Scopes Defined')
    )
  }
  ...
  if (request.params.teamId) {
    ...
    if (locatorPath) {
      ...
      if (typeof resolvedId === 'string') teamId = resolvedId
    }
  }
  ...
  if (!teamId || requiredBindles.length === 0) {
    return Promise.reject(
      new ResourceAccessError(
        403,
        'Bindle Security Check failed: Could not resolve Team ID or missing Required Bindles'
      )
    )
  }
  ...
  try {
    ...
    const effectivePermissions = BindleController.getEffectivePermissionSet(
      teamInfo,
    ...
    if (missingBindles.length === 0) return Promise.resolve(true)

    /* Access Denied */
    const owners = authoritativeTeam.users.map((u) => u.name)
    return Promise.reject(
      new ResourceAccessError(403, formatBindleAccessError(owners, missingBindles))
    )
  } catch (e) {
